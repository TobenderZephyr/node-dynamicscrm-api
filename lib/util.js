...
var domParser = new (require('@xmldom/xmldom').DOMParser)();
var fs = require('fs');
var parseString = require('xml2js').parseString;
var traverse = require('traverse');
var Serializer = require('./serializer.js');
var DynamicsError = require('./error');
...
try {
  ...
  request = function (opts, callback) {
    var encoded = Buffer.from(JSON.stringify(opts)).toString('base64');
    var execOpts = {
      maxBuffer: 1024 * 1000 * 1000,
    };

    exec(
      NEW_NODE_PATH + ' ' + __dirname + '/proxy_requester.js ' + encoded,
      execOpts,
      function (err, stdout, stderr) {
        if (err) {
          console.log('DYNAMICS CRM XRM CHILD PROC ERR', err);
          originalRequest(opts, callback);
        } else {
          var results = err ? { err: err } : JSON.parse(stdout);
...
var Util = function (settings) {
  ...
  if (
    settings.defaultUrlSuffix &&
    typeof settings.defaultUrlSuffix !== 'boolean'
  )
    throw new Error("'settings.defaultUrlSuffix' property must be a boolean.");
  if (settings.crmRegion && typeof settings.crmRegion !== 'string')
    throw new Error("'settings.crmRegion' property must be a string");
  ...
  settings.crmRegion = settings.crmRegion || 'crmna';

  // Define static properties
  var defaultUrlSuffix =
      settings.defaultUrlSuffix === false ? '' : '.api.crm.dynamics.com',
    ...
    parseResponse,
    //load templates once
    microsoftOnlineSaml = fs
      .readFileSync(__dirname + '/templates/microsoft_online_saml.xml')
      .toString(),
    authCreateDeviceMessage = fs
      .readFileSync(__dirname + '/templates/auth_create_device.xml')
      .toString(),
    authRequestDeviceTokenMessage = fs
      .readFileSync(__dirname + '/templates/auth_tokenrequest_device.xml')
      .toString(),
    authRequestSTSTokenMessage = fs
      .readFileSync(__dirname + '/templates/auth_tokenrequest_liveid.xml')
      .toString(),
    soapEnvelopeMessage = fs
      .readFileSync(__dirname + '/templates/soapMessage.xml')
      .toString(),
    soapHeaderMessage = fs
      .readFileSync(__dirname + '/templates/soapHeader.xml')
      .toString(),
    apiRetrieveMultipleMessage = fs
      .readFileSync(__dirname + '/templates/api_retrievemultiple.xml')
      .toString(),
    apiRetrieveMessage = fs
      .readFileSync(__dirname + '/templates/api_retrieve.xml')
      .toString(),
    apiCreateMessage = fs
      .readFileSync(__dirname + '/templates/api_create.xml')
      .toString(),
    apiUpdateMessage = fs
      .readFileSync(__dirname + '/templates/api_update.xml')
      .toString(),
    apiDeleteMessage = fs
      .readFileSync(__dirname + '/templates/api_delete.xml')
      .toString(),
    apiExecuteMessage = fs
      .readFileSync(__dirname + '/templates/api_execute.xml')
      .toString(),
    apiAssociateMessage = fs
      .readFileSync(__dirname + '/templates/api_asociate.xml')
      .toString(),
    apiDisassociateMessage = fs
      .readFileSync(__dirname + '/templates/api_disassociate.xml')
      .toString(),
    apiAssignMessage = fs
      .readFileSync(__dirname + '/templates/api_assign.xml')
      .toString(),
    apiAddToList = fs
      .readFileSync(__dirname + '/templates/api_addtolist.xml')
      .toString(),
    faultTextXpath =
      "//*[local-name()='Fault']/*[local-name()='Reason']/*[local-name()='Text']/text()",
    importLocationXpath =
      "//*[local-name()='import' and namespace-uri()='http://schemas.xmlsoap.org/wsdl/']/@location",
    authenticationTypeXpath =
      "//*[local-name()='Authentication' and namespace-uri()='http://schemas.microsoft.com/xrm/2011/Contracts/Services']/text()",
    issuerAddressXpath =
      "//*[local-name()='SignedSupportingTokens']/*[local-name()='Policy']/*[local-name()='IssuedToken']/*[local-name()='Issuer']/*[local-name()='Address']/text()",
  ...
  fetchEndpoints = function (cb) {
    ...
    request(options, function (err, res, body) {
      ...
      if (fault.length > 0)
        return cb(new DynamicsError(fault.toString(), body), null);

      location = xpath.select(importLocationXpath, resXml).map(function (attr) {
        return attr.value;
      })[0];
      ...
      if (location && location.length > 0) {
        ...
        request(opts, function (err, res, body) {
          ...
          if (faultImport.length > 0)
            return cb(new DynamicsError(faultImport.toString(), body), null);

          authenticationType = xpath
            .select(authenticationTypeXpath, resXmlImport)
            .toString();
          issuerAddress = xpath
            .select(issuerAddressXpath, resXmlImport)
            .toString();
          liveAppliesTo = xpath
            .select(liveAppliesToXpath, resXmlImport)
            .toString();
          identifier = xpath
            .select("//*[local-name()='Identifier']/text()", resXmlImport)
            .toString();
          keyType = xpath
            .select("//*[local-name()='KeyType']/text()", resXmlImport)
            .toString();
          keySize = xpath
            .select("//*[local-name()='KeySize']/text()", resXmlImport)
            .toString();
  ...
  getTokenUsingDeviceId = function (options, cb) {
    ...
    request(requestOptions, function (err, res, body) {
      ...
      cipherValue = xpath
        .select(
          "//*[local-name()='RequestedSecurityToken' and namespace-uri()='http://schemas.xmlsoap.org/ws/2005/02/trust']/*[name()='EncryptedData']/*[name()='CipherData']/*[name()='CipherValue']/text()",
          resXml
        )
        .toString();
  ...
  parseResponse = function (body, cb) {
    ...
    if (fault.length > 0) return cb(new DynamicsError(fault.toString(), body));
  ...
  executeSoapPost = function (options, action, template, body, cb) {
    ...
    if (settings.authType === 'ntlm') {
      ...
      request(reqOptions, function (err, res) {
        ...
        // console.log('reqOptions', JSON.stringify(reqOptions, null, 2))
      ...
      // console.log('MAKE REQUEST', requestOptions);
  ...
  authenticateUsingMicrosoftOnline = function (opts, cb) {
    var loginEndpoint = 'urn:' + settings.crmRegion + ':dynamics.com',
    ...
    request(options, function (err, res, body) {
      ...
      if (fault.length > 0)
        return cb(new DynamicsError(fault.toString(), body));
            ...
            if (faultDetails.length > 0)
              parseString(
                ...
                function (err, data) {
                  ...
                  return cb(new DynamicsError(fullMessage, body), null);
                }
              );
          });
        });
  ...
  authenticateUsingFederation = function (authOptions, cb) {
    fetchEndpoints(function (err, wsdlInfo) {
      ...
      wstrustFlowOptions = {
        wstrustEndpoint:
          authOptions.wstrustEndpoint || identifier + '/2005/usernamemixed',
  ...
  this.Authenticate = function (options, cb) {
    ...
    options.wstrustEndpoint =
      options.wstrustEndpoint || settings.wstrustEndpoint;
  ...
  this.RetrieveMultiple = function (options, cb) {
    // console.log('RetrieveMultiple options', JSON.stringify(options, null, 2))
    this.executePost(
      options,
      'RetrieveMultiple',
      apiRetrieveMultipleMessage,
  ...
  /*
    RetrieveEntity public and private methods
    */
  this.RetrieveEntity = function (options, cb) {
    this.executePost(
      ...
      'Execute',
      apiExecuteMessage,
      serializer.toXmlRetrieveEntity(options),
      cb
    );
  };
  ...
  this.RetrieveAttribute = function (options, cb) {
    this.executePost(
      ...
      'Execute',
      apiExecuteMessage,
      serializer.toXmlRetrieveAttribute(options),
      cb
    );
  };
  ...
  this.Assign = function (options, cb) {
    this.executePost(
      ...
      'Execute',
      apiAssignMessage,
      serializer.toXmlAssign(options),
      cb
    );
  };
  ...
  this.AddToList = function (options, cb) {
    this.executePost(
      ...
      'Execute',
      apiAddToList,
      serializer.toXmlAddToList(options),
      cb
    );
  };
