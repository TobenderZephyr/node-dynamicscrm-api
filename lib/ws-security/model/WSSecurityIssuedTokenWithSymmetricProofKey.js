...
var template = fs.readFileSync(
  path.join(
    __dirname,
    '/templates/WSSecurityIssuedTokenWithSymmetricProofKey.xml'
  ),
  'utf-8'
);
...
function WSSecurityIssuedTokenWithSymmetricProofKey(opts) {
  ...
  var signatureValue = crypto
    .createHmac('sha1', Buffer.from(signingKey, 'base64'))
    .update(signedInfo)
    .digest('base64');
  ...
  function calculatePSHA1(clientKey, serverKey, keySize) {
    var clientBytes = Buffer.from(clientKey, 'base64');
    var serverBytes = Buffer.from(serverKey, 'base64');

    var sizeBytes = keySize / 8;
    var sha1DigestSizeBytes = 160 / 8;

    var buffer1 = serverBytes;
    var buffer2 = Buffer.alloc(sha1DigestSizeBytes + serverBytes.length);
    var pshaBuffer = Buffer.alloc(sizeBytes);
    ...
    while (i < sizeBytes) {
      buffer1 = Buffer.from(
        crypto.createHmac('sha1', clientBytes).update(buffer1).digest(),
        'binary'
      );
      ...
      temp = Buffer.from(
        crypto.createHmac('sha1', clientBytes).update(buffer2).digest(),
        'binary'
      );
